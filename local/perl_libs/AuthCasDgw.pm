# AuthCasDgw.pm - CAS ticket validation
#----------------------------------------------------------------------
# $Id: //Tuxedo/RELEASE/Product/server/perl_libs/AuthCasDgw.pm#3 $
#----------------------------------------------------------------------
#
# DESCRIPTION:
#   This is a version of the free-source AuthCAS.pm module that is
#   adapted for DegreeWorks CAS integration.
#----------------------------------------------------------------------
...
sub validateST {
	...
	my $attrValue;
	my %userCas = ();
	$userCas{'casAuthenticatedUser'} = $user;
	$userCas{'xmlstring'} = $xmlstring;

	# DegreeWorks: Add each "cas:attribute" node into the userCas hash 
	# so that authcas script can find the User ID attribute.
	if(defined($xmldoc))
	{
		my @parent = $xmldoc->getElementsByTagName ("cas:attribute");
		{
			foreach my $parent(@parent)
			{
				#if ($parent->getNodeType == ELEMENT_NODE)
				{
					my $attrName = $parent->getAttribute('name');
					$userCas{$attrName} = $parent->getAttribute('value');
				}
			}
		}
	}
	...
	{
		$errors .= "Error: CAS xml response, xmldoc is undefined \n ";
		return undef;
	}
...
#use Data::Dumper; die '--'.$#$xml.': '.Dumper($xml);
	...
	# DegreeWorks: Parse the CAS response into a real XML::DOM doc.
	$xmlstring = join('', @$xml);
   ...
   if (length($xmlstring) > 0)
   {
	...
	{
		$errors .= "CAS xml response, xmlstring has no contents \n ";
		return undef;
	}

   return &_parse_xml($xmlstring);
}
...
"Unable to use LWP library, LWP::UserAgent required, install LWP (CPAN) first\n";
    ...
    $ssl_options{'SSL_verify_mode'} = 0x00
