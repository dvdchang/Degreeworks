...
<!-- $Id: //Tuxedo/RELEASE/Product/release/server/admin/xsl/fopaudits-aid.xsl#4 $ -->
...
<xsl:variable name="LabelProgressBar">      Progress Towards Degree</xsl:variable>
...
<xsl:template match="/">
	...
	<xsl:for-each select="Report/Audit">
		...
		<fo:static-content flow-name="xsl-region-before">
          ...
          <xsl:call-template name="tFinancialAidData"/> 
          <xsl:call-template name="tProgressBar"/>
          <xsl:call-template name="tCourseSignalsHelp" />
					<xsl:call-template name="tStudentAlerts"/>
					<xsl:call-template name="tAwardSummary"/>
					<xsl:call-template name="tBlocksAwardOnly"/>
          <xsl:if test="contains(/Report/@rptReportCode,'51')"> <!-- WEB51 and RPT51 -->
...
<xsl:include href="fop-tAwardSummary.xsl" />
<xsl:include href="fop-tFinancialAidData.xsl" /> 
